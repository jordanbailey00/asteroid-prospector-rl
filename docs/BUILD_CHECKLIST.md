...
- `M0`: Phase 0 (Pre-flight / bootstrap)
- `M1`: Phase 1 (Python reference env)
- `M2`: Phase 2 (Native core + bindings)
- `M2.5`: Phase 3 (Parity harness)
- `M3`: Phase 4 (Training + W&B)
- `M4`: Phase 5 (Eval + replay)
- `M5`: Phase 6 (Backend API)
- `M6`: Phase 7 (Frontend)
- `M6.5`: Phase 8 (Graphics + audio)
- `M7`: Phase 9 (Baselines + benchmarking)
- `M8`: Phase 10 (Performance + stability hardening)
...
### 0.1 [M0.1] Repo structure (create skeleton only)
...
### 0.3 [M0.3] "Hello env" stub (contract-only)
...
### 1.3 [M1.3] Integration tests (Tier 2)
...
### 2.1 [M2.1] Native core scaffolding
...
### 2.2 [M2.2] Implement C core reset + step
- `reset(seed)`:
...
### 2.4 [M2.4] Native testing harness
...
### 3.1 [M2.5.1] Parity runner tooling
...
### 4.1 [M3.1] PufferLib training integration
...
### 4.2 [M3.2] Windowing implementation
...
### 5.2 [M4.2] Replay recorder + formats
...
### 6.1 [M5.1] Run/metrics endpoints
Implement:
- `GET /api/runs`
- `GET /api/runs/{run_id}`
...
### 6.3 [M5.3] Human-play session endpoints
Implement:
...
### 6.4 [M5.4] CORS + deployment config
...
### 7.4 [M6.4] Historical analytics page
...
### 8.1 [M6.5.1] Source asset ingestion (authoritative)
...
### 8.2 [M6.5.2] Runtime asset packaging for frontend
...
### 8.3 [M6.5.3] Manifest completion (hard requirement)
...
### 8.4 [M6.5.4] Frontend wiring (replay + play)
...
### 8.5 [M6.5.5] Validation and regression checks
...
### 10.3 [M8.3] Replay + API robustness
...
### 11.1.1 [M9.1.a] Performance-critical execution order (must follow)
- P1 native PPO env path:
  - add `ppo_env_impl` support (`reference|native|auto`)
  - default to `auto` for profiling runs
  - validate native path contracts in tests
- P2 callback overhead reduction:
  - remove per-env Python callback loop in hot stepping path
  - introduce batch/aggregated metric processing
- P3 C batch step/reset APIs:
  - add native `step_many`/`reset_many` interfaces
  - expose batched Python bridge
  - add batch-vs-single parity tests
- P4 native-core micro-optimizations:
  - optimize observation packing and critical update loops
  - keep determinism checks and contract tests green

Reference plan: `docs/PERFORMANCE_BOTTLENECK_PLAN.md`

### 11.2 [M9.2] W&B-to-website analytics integration
...
### 11.3 [M9.3] Vercel deployment path
...
### 11.4 [M9.4] Public Replay/Play UX realignment
- Introduce shared viewport-first shell for `/` and `/play`:
  - large gameplay viewport as primary surface
  - compact right-side HUD for critical gameplay metrics
- Simplify public control surfaces:
  - keep observer/player essentials visible
  - move advanced transport/session tuning into collapsed advanced panels
- Add explicit pilot onboarding on `/play`:
  - action groups by gameplay intent
