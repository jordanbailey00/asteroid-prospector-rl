...
Current focus: M3 trainer backend completion (PufferLib integration)

## Current state

- Frozen interface status: unchanged (`OBS_DIM=260`, `N_ACTIONS=69`, action indexing `0..68`, reward definition unchanged).
- Build health: local checks passing via `tools/run_checks.ps1` (`pytest -q` currently 29 passed).
...
| M3 - Training + window metrics | In progress | Windowed trainer loop, checkpoint cadence, `windows.jsonl` emission, optional W&B logging adapter, and M3 tests | pending (this commit) |
...
1. Replace random trainer backend in `training/train_puffer.py` with real PufferLib PPO wiring.
2. Persist run metadata fields required by M5/M6 (`latest_window`, run status, checkpoint/replay pointers).
3. Implement M4 eval runner and replay generation per window.
4. Start M5 API server for run/metrics/replay/play-session endpoints.

## Active risks and blockers

- Main risk: current trainer backend is random-policy scaffolding; no learning yet.
- Main blocker for MVP: M3 still needs true PPO training plus M4-M6 implementation.
