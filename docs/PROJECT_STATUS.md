...
- Active milestone: M4 eval + replay generation.
...
| M3 - Training + window metrics | Complete | Windowed trainer loop, checkpoint cadence, `windows.jsonl`, optional W&B logging, live `run_metadata.json`, and Dockerized Linux `puffer_ppo` backend with PufferLib PPO smoke validation (3 windows) | pending (this commit) |
...
1. Implement M4 eval runner and replay generation per window.
...
- Main blocker for MVP: M4-M6 are not implemented yet.
