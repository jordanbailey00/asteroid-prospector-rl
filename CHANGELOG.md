...
- Hardened replay websocket reliability with backend internal-error envelopes (`type=error`, `status_code=500`), prelude flush yield, and explicit success-path close handshake (`code=1000` after a short flush window), plus retryable deployment smoke checks via `--ws-check-attempts` (default `3`), including regression coverage in `tests/test_server_api.py` and `tests/test_smoke_m9_deployment.py`.
...
- Refreshed M9 release-cut deployment evidence on 2026-03-04 with a passing local strict smoke artifact (`artifacts/deploy/m9-smoke-strict-20260304-chunk1-run1.json`) and passing manual CI smoke run `22655188824` artifact (`artifacts/deploy/ci-m9-smoke-22655188824/m9-deployment-smoke-22655188824/m9-smoke-22655188824.json`).
- Hardened throughput matrix calibration with trainer-backend coverage gating in `tools/run_throughput_matrix.py` (`--required-trainer-backends`, `--fail-on-coverage-gap`) plus coverage telemetry (`coverage_pass`, `coverage_gaps`, `successful_trainer_backends`) and regression tests in `tests/test_run_throughput_matrix.py`; captured Chunk 2 evidence in `docs/M9_CHUNK2_THROUGHPUT_EXECUTION_20260304.md`.
- Added explicit CORS drift guardrails to deployment smoke (`backend-cors-simple`, `backend-cors-preflight`) in `tools/smoke_m9_deployment.py` with `--cors-origin` override support, plus regression coverage in `tests/test_smoke_m9_deployment.py`; captured Chunk 3 strict smoke evidence in `docs/M9_CHUNK3_DRIFT_GUARDRAILS_EXECUTION_20260304.md` and `artifacts/deploy/m9-smoke-strict-20260304-chunk3-run1.json`.
...
- Executed M9 dry-run split deployment (Railway backend + Vercel frontend) and captured smoke evidence in `docs/M9_DEPLOYMENT_EVIDENCE_20260303.md` plus `artifacts/deploy/m9-smoke-live-20260303.json` and CI artifact `artifacts/deploy/ci-m9-smoke-22639886521/m9-smoke-22639886521.json`.
