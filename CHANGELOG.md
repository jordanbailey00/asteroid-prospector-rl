...
### Added
- Hardened replay websocket reliability with backend internal-error envelopes (`type=error`, `status_code=500`) and retryable deployment smoke checks via `--ws-check-attempts` (default `3`), including regression coverage in `tests/test_server_api.py` and `tests/test_smoke_m9_deployment.py`.
